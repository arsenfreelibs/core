...
pub async fn get_securejoin_qr(context: &Context, chat: Option<ChatId>) -> Result<String> {
    ...
    let qr = if let Some(chat) = chat {
        ...
        if chat.typ == Chattype::OutBroadcast {
            ...
            format!(
                "https://alt-chat.me/#{fingerprint}&v=3&x={grpid}&j={invitenumber}&s={auth}&a={self_addr_urlencoded}{r_param}&n={self_name_urlencoded}&b={chat_name_urlencoded}",
            )
        } else {
            format!(
                "https://alt-chat.me/#{fingerprint}&v=3&x={grpid}&i={invitenumber}&s={auth}&a={self_addr_urlencoded}{r_param}&n={self_name_urlencoded}&g={chat_name_urlencoded}",
            )
        }
    } else {
        ...
        format!(
            "https://alt-chat.me/#{fingerprint}&v=3&i={invitenumber}&s={auth}&a={self_addr_urlencoded}{r_param}&n={self_name_urlencoded}",
        )
    };
