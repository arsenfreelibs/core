...
async fn test_outgoing_undecryptable() -> Result<()> {
    ...
    assert!(
        dev_msg
            .text
            .starts_with("⚠️ It seems you are using Alt Chat on multiple devices that cannot decrypt each other's outgoing messages. To fix this, on the older device use \"Settings / Add Second Device\" and follow the instructions. (Error:")
    );
